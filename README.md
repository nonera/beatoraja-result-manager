...
- **ギャラリー** タブ: スクショ選択・投稿
- **データベース** タブ: 全スクショの一覧（サムネイル・日付・タイトル・記号・レベル・ランク・ランプ・投稿表記・状態）
  - **サムネイル** … 各スクショの縮小画像。フィルタ行の「サムネイル」チェックで非表示にできます
  - **記号** … ★ / sl / δ など（表記号のみ）
  - **レベル** … 12 / ??? など。表外譜面は記号が空で LEVEL の数字のみ
  - **ランク / ランプ** … beatoraja に準じた色分けバッジで表示
  - **投稿表記** … 編集可（例: `★3/sl3`）。セル直接編集または「候補から投稿表記を選択」
  - **状態** … キャプチャ種別と投稿済みサービス（`X` / `Discord`）のバッジ
...
設定画面の **外観** で、ダーク（既定）とライトを切り替えられます。保存するとメイン画面を開き直して配色が切り替わります。
